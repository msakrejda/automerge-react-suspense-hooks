 ...
 * @returns 
 */
export function resolveHandle<T>(id: Id, repo: Repo, cache: PromiseCache<Id, DocHandle<T>>) {
  return cache.resolve(id, (id, resolve, reject) => {
    if (!id) {
      reject(new Error("missing id"));
    }
    const handle = repo.find<T>(id);
    handle.doc().then(() => resolve(handle)).catch(reject);
  });
}
 ...
 * @returns 
 */
export function resolveHandles<T>(ids: Id[], repo: Repo, cache: PromiseCache<Id, DocHandle<T>>) {
